...
class HelenAAM(AAM):
    ...
    def __init__(self, path_to_data=HELEN_DATA_FOLDER,
                 model_type=HolisticAAM, basename='helen_aam', verbose=True):

        super(HelenAAM, self).__init__(
            path_to_data, model_type, basename, verbose)

    def _crop_grayscale_images(
            self, filepath, crop_percentage, max_images=None):

        images = []

        for i in mio.import_images(
                filepath, max_images=max_images, verbose=self.verbose):
    ...
    def load_data(self, crop_percentage=0.1, max_images=None):
        ...
        self.train_set = self._crop_grayscale_images(train_path,
                                                     crop_percentage,
